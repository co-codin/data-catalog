...
from app.crud.crud_model_relation import read_relations_by_group_id, read_relation_by_guid, create_model_relation, update_model_relation, delete_model_relation
...
@router.post('/')
async def add_model_relation(relation_in: ModelRelationIn, session=Depends(db_session), user=Depends(get_user)):
    guid = await create_model_relation(relation_in, session)

    return {'guid': guid}

@router.put('/{guid}')
async def update(guid: str, relation_update_in: ModelRelationUpdateIn, session=Depends(db_session), user=Depends(get_user)):
