...
from app.crud.crud_model_attitude import read_attitudes_by_resource_id, read_attitude_by_guid, create_model_attitude, \
    delete_model_attitude
...
@router.get('/{guid}')
async def get_model_attitude(guid: str, session=Depends(db_session), token=Depends(get_token)):
    return await read_attitude_by_guid(guid, token, session)
